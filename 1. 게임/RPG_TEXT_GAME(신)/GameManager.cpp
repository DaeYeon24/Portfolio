...
void GameManager::MonsterLoad()
{
	...
	string tmp, slot, tmp_Name, tmp_Material;
	...
	{
		load >> tmp_Name >> tmp_Level >> tmp_Att >> tmp_Def >> tmp_FullHp >> tmp_Exp >> tmp_MaxCost >> tmp_Gold >> tmp_Material;
		m_Monster_List[i].MonsterSetting(tmp_Name, tmp_Level, tmp_Att, tmp_Def, tmp_FullHp, tmp_Exp, tmp_MaxCost, tmp_Gold, tmp_Material);
	}
	load.close();
}
...
bool GameManager::WinCheck()
{
	...
	if (m_User->GetHp() <= 0 || m_Monster_List[m_iSelect_Index].GetHp() <= 0)
	{
		...
		if (m_Monster_List[m_iSelect_Index].GetHp() <= 0)
		{
			...
			m_Monster_List[m_iSelect_Index].GetMaterial(m_User);
			m_User->LevelUp();
		}
...
void GameManager::WeaponLoad()
{
	...
	load.close();
	m_Weapon_List = new Item * [m_iWeapon_Count];
	load.open("WeaponList.txt");
	for (int i = 0; i < m_iWeapon_Count; i++)
	{
		...
		{
		case BOW:
			m_Weapon_List[i] = new Bow(tmp_name, (TYPE)tmp_type, tmp_att, tmp_price);
			break;
		case SWORD:
			m_Weapon_List[i] = new Sword(tmp_name, (TYPE)tmp_type, tmp_att, tmp_price);
			break;
		case WAND:
			m_Weapon_List[i] = new Wand(tmp_name, (TYPE)tmp_type, tmp_att, tmp_price);
			break;
		case SPEAR:
			m_Weapon_List[i] = new Spear(tmp_name, (TYPE)tmp_type, tmp_att, tmp_price);
			break;
		}
	}
	load.close();
...
void GameManager::WeaponShop()
{
	...
	for (int i = 0; i < m_iWeapon_Count; i++)
	{
		if (m_Weapon_List[i]->GetType() == Weapon_type)
		{
			...
			{
			case BOW:
				tmp[list] = new Bow(m_Weapon_List[i]->GetName(), m_Weapon_List[i]->GetType(), m_Weapon_List[i]->GetAtt(), m_Weapon_List[i]->GetPrice());
				break;
			case SWORD:
				tmp[list] = new Sword(m_Weapon_List[i]->GetName(), m_Weapon_List[i]->GetType(), m_Weapon_List[i]->GetAtt(), m_Weapon_List[i]->GetPrice());
				break;
			case WAND:
				tmp[list] = new Wand(m_Weapon_List[i]->GetName(), m_Weapon_List[i]->GetType(), m_Weapon_List[i]->GetAtt(), m_Weapon_List[i]->GetPrice());
				break;
			case SPEAR:
				tmp[list] = new Spear(m_Weapon_List[i]->GetName(), m_Weapon_List[i]->GetType(), m_Weapon_List[i]->GetAtt(), m_Weapon_List[i]->GetPrice());
				break;
			}
			list++;
		}
	...
	while (1)
	{
		...
		{
			Item* bought;
			switch (tmp[(menuselect + (page * 4)) - 1]->GetType())
			{
			case BOW:
				bought = new Bow(tmp[(menuselect + (page * 4)) - 1]);
				break;
			case SWORD:
				bought = new Sword(tmp[(menuselect + (page * 4)) - 1]);
				break;
			case WAND:
				bought = new Wand(tmp[(menuselect + (page * 4)) - 1]);
				break;
			case SPEAR:
				bought = new Spear(tmp[(menuselect + (page * 4)) - 1]);
				break;
			}
			m_User->BuyWeapon(bought);
			list = page * 4;
			line = 1;
			menulen = 0;
			continue;
...
void GameManager::Monster_Info()
{
	...
	for (int i = 0; i < m_iMonster_Count; i++)
	{
		m_Monster_List[i].ShowUnit(line, true);
		line += 6;
	}
	getch();
}
